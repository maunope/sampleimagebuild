...
build {
  ...
  provisioner "shell" {
    inline = [
      ...
      "# CORRECTED: Use a more robust method for user creation to ensure password authentication works correctly.",
      "mysql -u root -e \"CREATE USER '$${DB_USERNAME}'@'%' IDENTIFIED WITH mysql_native_password BY '$${DB_PASSWORD}';\"",
      "mysql -u root -e \"GRANT ALL PRIVILEGES ON *.* TO '$${DB_USERNAME}'@'%' WITH GRANT OPTION; ALTER USER 'root'@'localhost' IDENTIFIED BY '$${ROOT_PASSWORD}'; FLUSH PRIVILEGES;\"",
